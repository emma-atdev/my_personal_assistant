...
```
PART 1 — 전체 구조
  backend/app.py                   ← 실제 서버 진입점. 전체 흐름 파악
  main.py                          ← CLI 버전 (미사용). 에이전트 흐름만 단순하게 파악용

PART 2 — 에이전트 (핵심)
  agent/orchestrator.py            ← 모든 것이 연결되는 중심
  agent/subagents/research.py      ← 가장 단순 (구조 파악)
  agent/subagents/note.py
  agent/subagents/file.py
  agent/subagents/github.py
  agent/subagents/cron.py
  agent/subagents/code.py          ← 가장 복잡

PART 3 — 툴 (에이전트가 호출하는 함수들)
  tools/memory.py                  ← 가장 단순. docstring 패턴 파악
  tools/search.py → tools/papers.py → tools/changelog.py
  tools/notion_tools.py → tools/github_tools.py
  tools/calendar_tools.py → tools/cost_tracker.py
  tools/local_file.py → tools/conversations.py

PART 4 — 인프라 (툴이 의존하는 기반)
...
PART 7 — 프론트엔드
  frontend/app.py                  ← main.py의 async + 스트리밍 확장 버전
```
...
### `main.py`
- **현재 실제 사용 안 함** — Streamlit 전환 전 CLI 버전. 실제 동작은 `frontend/app.py` + `backend/app.py`
- 구조 파악용으로만 읽기. `create_orchestrator()` → `agent.invoke()` → HITL 처리 흐름이 가장 단순하게 담겨있음
- **포인트**: `state.next` 유무로 HITL 감지. Streamlit 버전(`frontend/app.py`)과 나중에 비교하며 보기
...
2. `_get_checkpointer()` — 단기 기억 설정 (MemorySaver vs AsyncPostgresSaver)
...
- **포인트**: `description`이 라우팅 기준. 라우팅 코드는 없음 — LLM이 판단
...
### `agent/subagents/cron.py`
- 브리핑/리포트 생성 전용. 구조는 research와 동일
...
### `tools/papers.py`
- ArXiv / HuggingFace Daily Papers에서 논문 수집
- **포인트**: httpx.Client는 컨텍스트 매니저, arxiv.Client는 싱글턴인 이유

### `tools/changelog.py`
- `append_changelog` 내부에서 `sync_changelog_to_notion`을 직접 호출
- **포인트**: 툴이 툴을 호출하는 패턴, 예외를 조용히 무시하는 이유
...
- **포인트**: 실제 파일 읽기는 여기서 안 함. MCP 서버에 위임
...
- orchestrator와 file 서브에이전트 양쪽 system_prompt에 동적으로 주입

### `utils/logger.py`
- `AgentLoggingHandler` — LangChain 콜백으로 LLM 호출/툴 실행을 자동 로깅
- **포인트**: 에이전트 이름(`kwargs["name"]`)으로 어떤 서브에이전트가 호출됐는지 구분
...
- deny vs ignore 차이: deny는 파일 읽기 자체를 차단 / ignore는 목록에서만 숨김
